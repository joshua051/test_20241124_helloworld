...
namespace IronSand.UI
{
    public sealed class PrototypeHUD : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            GUI.Box(new Rect(16f, 16f, 505f, 286f), "IRON SAND ARENA - COMBAT VERTICAL SLICE 2.0 / UNVALIDATED");
            string hp = player == null ? "--" : $"{Mathf.CeilToInt(player.Health)} / {Mathf.CeilToInt(player.MaxHealth)}";
            string poise = player == null ? "--" : $"{Mathf.CeilToInt(player.Poise)} / {Mathf.CeilToInt(player.MaxPoise)}";
            string crowd = favor == null ? "--" : $"{favor.Favor} / {favor.MaxFavor}";
            string pending = crowdRewards == null ? "--" : crowdRewards.PendingRewards.ToString();
            ...
            string state = player == null ? "--" : player.IsExecuting ? "EXECUTION" : player.IsAttacking ? $"{player.AttackPhase} Chain {player.AttackChainStep}" : player.Guarding ? "GUARD" : "FREE";
            GUI.Label(new Rect(32f, 46f, 470f, 20f), $"Health: {hp}   Poise: {poise}");
            GUI.Label(new Rect(32f, 68f, 470f, 20f), $"State: {state}   Dodge: {dodge}");
            GUI.Label(new Rect(32f, 90f, 470f, 20f), $"Weapon: {weaponName}   Durability: {durability}");
            GUI.Label(new Rect(32f, 112f, 470f, 20f), $"Style: {score}");
            GUI.Label(new Rect(32f, 134f, 470f, 20f), $"Crowd Favor: {crowd}   Rewards ready: {pending}");
            GUI.Label(new Rect(32f, 156f, 470f, 20f), $"Wave: {wave}   Enemies: {enemies}");
            GUI.Label(new Rect(32f, 180f, 470f, 92f), "LMB/RMB Attack (buffer during late swing/recovery) | Q Guard/Perfect\nSpace Dodge | Tab Lock | E Swap | G Throw | F Execute | C Crowd Appeal\nWASD Move | Shift Sprint | Esc Pause | R Restart");
            if (crowdRewards != null && crowdRewards.PendingRewards > 0)
                GUI.Box(new Rect(Screen.width - 260f, 20f, 240f, 38f), $"CROWD REWARD READY x{crowdRewards.PendingRewards}  [C]");
            DrawEnemyMarkers();
        ...
        private void DrawEnemyMarkers()
        {
            ...
            foreach (var enemy in director.AliveEnemies)
            {
                ...
                if (!locked && !enemy.IsAttackCommitted && !enemy.ExecutionReady) continue;
                Vector3 screen = gameplayCamera.WorldToScreenPoint(enemy.transform.position + Vector3.up * 1.4f);
                if (screen.z <= 0f) continue;
                float y = Screen.height - screen.y;
                string label = enemy.ExecutionReady ? "EXECUTE [F]" : enemy.IsAttackCommitted ? "ATTACK INCOMING" : $"LOCK  HP {enemy.Health:0}  POISE {enemy.Poise:0}";
                GUI.Box(new Rect(screen.x - 82f, y - 14f, 164f, 28f), label);
                if (enemy.IsAttackCommitted)
                    GUI.Box(new Rect(screen.x - 82f, y + 14f, Mathf.Max(1f, 164f * enemy.WindupProgress), 8f), "");
            }
        }
    }
}
