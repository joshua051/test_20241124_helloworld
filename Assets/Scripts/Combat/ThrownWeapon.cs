...
namespace IronSand.Combat
{
    ...
    public sealed class ThrownWeapon : MonoBehaviour
    {
        ...
        public static bool Spawn(Vector3 origin, Vector3 direction, WeaponArchetype weapon, int remainingDurability, Combatant source)
        {
            ...
            root.name = $"Thrown_{weapon}";
            root.transform.position = origin;
            root.transform.localScale = WeaponPickup.GetPickupScale(weapon) * 0.82f;
            Rigidbody rigidbody = root.AddComponent<Rigidbody>();
            rigidbody.mass = weapon == WeaponArchetype.Mace ? 2.2f : weapon == WeaponArchetype.Axe ? 1.7f : 1.1f;
            rigidbody.collisionDetectionMode = CollisionDetectionMode.ContinuousDynamic;
            Collider projectileCollider = root.GetComponent<Collider>();
            if (source != null && projectileCollider != null)
            {
                foreach (Collider ownerCollider in source.GetComponentsInChildren<Collider>())
                    Physics.IgnoreCollision(projectileCollider, ownerCollider, true);
            }
            ThrownWeapon thrown = root.AddComponent<ThrownWeapon>();
            thrown.archetype = weapon;
            thrown.durability = remainingDurability;
            thrown.owner = source;
            thrown.body = rigidbody;
            rigidbody.linearVelocity = direction.normalized * (weapon == WeaponArchetype.Spear ? 17f : 13f) + Vector3.up * 1.8f;
            rigidbody.angularVelocity = new Vector3(8f, 14f, 11f);
            return true;
        }
        ...
        {
            if (resolved) return;
            EnemyGladiator enemy = collision.collider.GetComponentInParent<EnemyGladiator>();
            if (enemy != null && enemy != owner && !enemy.IsDead)
            {
                WeaponStats stats = WeaponCatalog.Get(archetype);
                Vector3 direction = body != null && body.linearVelocity.sqrMagnitude > 0.01f ? body.linearVelocity.normalized : transform.forward;
            ...
            ResolveToPickup(point + Vector3.up * 0.25f);
        }
        private void ResolveToPickup(Vector3 position)
        {
            if (resolved) return;
            resolved = true;
            int remaining = Mathf.Max(0, durability - 2);
            position.y = 0.35f;
            if (remaining > 0) WeaponPickup.Spawn(position, archetype, remaining);
            Destroy(gameObject);
        }
    }
}
