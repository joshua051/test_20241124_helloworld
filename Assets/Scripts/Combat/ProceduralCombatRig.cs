...
namespace IronSand.Combat
{
    public sealed class ProceduralCombatRig : MonoBehaviour
    {
        ...
        {
            EnsureBuilt(); hitReaction = Mathf.MoveTowards(hitReaction, 0f, Time.deltaTime * 4.5f);
            float walk = Mathf.Sin(Time.time * 9f) * 24f * locomotion; float torsoYaw = 0f; float torsoRoll = -hitReaction * 16f - vulnerableAmount * 12f;
            ...
            {
                float signed = attackKind == AttackKind.Heavy ? 1f : -1f; float swing = Mathf.Sin(attackProgress * Mathf.PI);
            ...
            leftArm.localRotation = Quaternion.Euler(leftX, 0f, leftZ); rightArm.localRotation = Quaternion.Euler(rightX, 0f, rightZ);
            leftLeg.localRotation = Quaternion.Euler(walk, 0f, 0f); rightLeg.localRotation = Quaternion.Euler(-walk, 0f, 0f);
            if (shield != null) shield.gameObject.SetActive(playerTeam);
        }
        private void EnsureBuilt()
        {
            if (built) return; built = true;
            MeshRenderer rootRenderer = GetComponent<MeshRenderer>(); if (rootRenderer != null) rootRenderer.enabled = false;
            Transform oldVisual = transform.Find("BodyVisual"); if (oldVisual != null) { Renderer renderer = oldVisual.GetComponent<Renderer>(); if (renderer != null) renderer.enabled = false; }
            ...
            leftLeg = CreatePart("Rig_LeftLeg", PrimitiveType.Cube, transform, new Vector3(-0.22f, -0.78f, 0f), new Vector3(0.28f, 0.82f, 0.30f));
            rightLeg = CreatePart("Rig_RightLeg", PrimitiveType.Cube, transform, new Vector3(0.22f, -0.78f, 0f), new Vector3(0.28f, 0.82f, 0.30f));
            weaponSocket = new GameObject("WeaponSocket").transform; weaponSocket.SetParent(rightArm, false); weaponSocket.localPosition = new Vector3(0f, -0.52f, 0.16f);
            shield = CreatePart("Buckler", PrimitiveType.Cylinder, leftArm, new Vector3(0f, -0.32f, 0.28f), new Vector3(0.46f, 0.08f, 0.46f)); shield.localRotation = Quaternion.Euler(90f, 0f, 0f);
            ApplyTeamColor();
        }
        ...
        {
            GameObject part = GameObject.CreatePrimitive(primitive); part.name = name; part.transform.SetParent(parent, false); part.transform.localPosition = localPosition; part.transform.localScale = scale;
            Collider collider = part.GetComponent<Collider>(); if (collider != null) { collider.enabled = false; Destroy(collider); }
            return part.transform;
        }
        private void ApplyTeamColor()
        {
            if (!built) return; Color color = playerTeam ? new Color(0.28f, 0.44f, 0.62f) : new Color(0.58f, 0.28f, 0.20f);
            foreach (Renderer renderer in GetComponentsInChildren<Renderer>(true)) { if (renderer.transform.name.StartsWith("WeaponVisual_")) continue; if (renderer.material != null) renderer.material.color = color; }
        }
    }
}
